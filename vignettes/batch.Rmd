...
To start, read in all of the PDF files from a directory called "pdf", and save the bibr files in a directory called "bibr". 

```{r}
#| eval: false

convert_bibr(filename = "pdf", 
             save_path = "bibr")
```
...
convert_grobid(filename = "pdf", 
               save_path = "xml", 
               api_url = "http://localhost:8070")
```

Convert the grobid XML files to bibr format using `grobid_to_bibr()`. Save them in a directory called "bibr".

```{r}
#| eval: false
grobid_to_bibr(xml_file = "xml", 
               save_path = "bibr")
```

### Read in converted files

After you convert your papers to bibr format, read in the bibr files to metacheck and save in an object called `papers`. 
...
```{r}
bib <- paper$bib
bib$authors <- unclass(bib$authors)
