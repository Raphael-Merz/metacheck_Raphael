...
The function `osf_api_calls()` lets you reset and retrieve the number of API calls made since the last reset. You can see that the project `r links$href[[1]]` had `r sum(all_contents$osf_type == "nodes")` nodes and `r sum(all_contents$osf_type == "files")` files, which required `r n_calls` API calls. 
